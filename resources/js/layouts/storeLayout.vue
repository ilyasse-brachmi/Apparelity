...
import type { Category, ProductResponse } from "@/types/index"
...
onMounted(async () => {
	...
	$AppAxios.get('/api/product')
		.then((response) => {
			maxPrice.value = response.data.reduce((max : number, item: ProductResponse) => { return (item.original.price > max) ? item.original.price : max },0);
	})
})
const nameValue = ref('')
...
const FillCategorie = (clickedCategorie: { id: number; name: string })=>{
	categorie.value = clickedCategorie
}
const price = ref<number>(0)
const FillPrice = (max: number)=>{
	price.value = max
}
</script>
<template lang="pug">
Navbar(@sidebar-toggle="sidebarToggle")
Sidebar(@categorieClicked="FillCategorie" @priceClicked="FillPrice" :categories="categories" :sidebarToggle="opened" :price="maxPrice")
	...
	div(class="shadow-md bg-gray-50 py-4 w-full h-fit min-h-[90vh]")
		slot(name="cards" :categorie="{id: categorie?.id , name: categorie?.name}" :price="price")
</template>