...
const test = (name: string) =>{
  router.push({ name: 'Store', query: { name: name } })
}
</script>
<template lang="pug">
  ...
  div(class="container mx-auto p-8 lg:p-16")
    ...
    div(class="flex justify-center my-8 sm:my-16")
      div(class="flex items-center justify-between w-full px-2 sm:px-4 lg:px-8 py-2 sm:py-4 border-2 border-black rounded-full max-w-[60rem]")
        input(type="text" placeholder="Enter a product name" @keyup.enter="test(nameValue)" v-model="nameValue" class="bg-transparent px-4 py-2 text-sm sm:text-base md:text-lg outline-none border-none w-full")
